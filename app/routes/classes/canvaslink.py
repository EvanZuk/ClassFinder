...
def canvaslink(user):
    ...
    for course in user.classes:
        app.logger.debug(
            f"Checking if course {
                course.name} is linked to a canvas course."
        )
        if course.canvasid is None:
            app.logger.debug(
                f"Course {
                    course.name} is not linked to a canvas course."
            )
            ...
            app.logger.debug(
                f"Course {
                    course.name} is already linked to a canvas course: {
                    course.canvasid}"
            )
    if len(newcourses) == 0:
        app.logger.debug(
            f"User {
                user.username} has no courses to link to canvas."
        )
        ...
        if course.canvasid is None:
            if request.json.get(str(course.id)) is not None:
                if request.json.get(str(course.id)).isdigit():
                    ...
                    app.logger.debug(
                        f"Course {
                            course.name} was not linked to a canvas course by {
                            user.username} because the canvas course id was not a number."
                    )
            else:
                app.logger.debug(
                    f"Course {
