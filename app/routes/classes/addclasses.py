...
from app.utilities.classes import add_class, add_user_to_class, check_if_class_exists, get_course, check_if_user_in_class, get_periods_of_user_classes, neededperiods
...
def addclasses(user):
    if len(get_periods_of_user_classes(user)) == len(neededperiods):
        app.logger.debug(f"User already has all of their classes. ({len(get_periods_of_user_classes(user))}/{len(neededperiods)})")
        return redirect('/')
    return render_template('addcourses.html', neededperiods=[period for period in neededperiods if period not in get_periods_of_user_classes(user)])

@app.route('/addclasses', methods=['POST'])
@verify_user(onfail=({'status': 'error', 'message': 'You must be logged in to do that.'}, 401))
def addclasses_post(user):
    if len(get_periods_of_user_classes(user)) == len(neededperiods):
        return error_response("You already have all of your classes."), 400
    if len(request.json) % 5 != 0:
        return error_response("Make sure you copied your classes from infinite campus, and that you have copied everything."), 400
    ...
    for course in classes:
        ...
        if newcourse['period'] in get_periods_of_user_classes(user):
            app.logger.debug(f"User already has a class in period {newcourse['period']}")
            continue
        ...
        if check_if_class_exists(newcourse['room'], newcourse['period']):
            continue
        createdclass = add_class(newcourse['name'], newcourse['period'], newcourse['room'], created_by=user.username)
