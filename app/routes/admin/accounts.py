...
def login_as(user, username):
    ...
    if loguser:
        ...
        response = success_response("Logged in as user.") if request.method == 'POST' else redirect(url_for('dashboard'))
        response.set_cookie('token', token.token, httponly=True, samesite='Strict', secure=True, max_age=600)
        response.set_cookie('admin_token', request.cookies.get('token'), httponly=True, samesite='Strict', secure=True, max_age=604800)
        return response
    return error_response("User not found."), 404
...
def create_account_post(user):
    ...
    app.logger.info(f"Creating account for {username} with email {email} by {user.username}.")
