...
def get_user_current_period(user: User):
    ...
    if currentcourse is None:
        ...
        return {"lunch": None, "period": current_period["period"], "end": current_period["end"], "start": current_period["start"], "course": None}
    if lunchtimes[currentcourse.lunch]["start"] <= current_time <= lunchtimes[currentcourse.lunch]["end"]:
        ...
        return {"lunch": currentcourse.lunch, "period": "Lunch", "end": lunchtimes[currentcourse.lunch]["end"], "start": lunchtimes[currentcourse.lunch]["start"], "course": currentcourse}
...
def get_today_courses(user: User):
    ...
    app.logger.debug(f"User {user.username} courses for today: {[course.name for course in newcourses]}")
    return newcourses

def add_class(name: str, period: int, room: str, created_by: str):
    newclass = Class(id=f"{room}p{period}", name=name, room=room, period=period, created_by=created_by)
